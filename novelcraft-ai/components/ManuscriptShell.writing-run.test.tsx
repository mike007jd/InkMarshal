...
describe('ManuscriptShell compact writing-run status', () => {
  ...
  it('keeps the slow-planning copy owned solely by WritingRunStatus while the empty centre keeps the blueprint description', () => {
    renderShell({
      chapters: [],
      writingRunState: liveRun({
        phase: 'planning',
    ...
    // The empty-draft centre keeps the main blueprint planning narration…
    expect(screen.getByText('Planning the chapter blueprint…')).toBeTruthy();
    expect(
      screen.getByText(/shaping chapters from your Story Deck/),
    ).toBeTruthy();
    // …and the "still planning" wait hint exists, but every occurrence lives
    // inside a WritingRunStatus (role=status) — never duplicated in the
    // empty centre.
    const hints = screen.getAllByText(/Model still planning/);
