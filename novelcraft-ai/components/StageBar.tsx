...
export interface StageBarProps {
  ...
  /** True while a writing stream is in flight — surfaces Pause. */
  isStreaming?: boolean;
  onPauseWriting?: () => void;
...
export function buildStageBarSteps(
  ...
  const currentIndex = stage === 'discovery_interview'
    ...
    : stage === 'ready_for_greenlight'
      ? 2
      : 3;
  const writingDone = stage === 'whole_book_unification' || stage === 'completed';
...
export function StageBar({
  ...
  const steps = buildStageBarSteps(stage, stepLabels);
  ...
  const isExportReady = stage === 'whole_book_unification' || stage === 'completed';
          ...
          {isWritingStage && (
            <>
              ...
              {isStreaming && onPauseWriting && (
                <StopStreamingButton onStop={onPauseWriting} label={t.writingPause} />
              )}
            </>
          )}
