...
import { isWritingRunBusyPhase, type WritingRunState } from '@/lib/writing-session';
...
export function WritingRunStatus({ state, controls, nowMs, density, className }: WritingRunStatusProps) {
  ...
  const busy = isWritingRunBusyPhase(state.phase);
    ...
    return (
      <div className={cn('relative', className)}>
        ...
        {detailsOpen && (
          <div
            id={detailsId}
            className="absolute right-0 top-full z-20 mt-1 flex w-64 flex-col gap-1.5 rounded-md border border-book-border bg-book-bg-card p-3 shadow-lg"
          >
            {detailRows}
          </div>
        )}
