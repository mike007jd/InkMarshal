...
4. **The stop-stream control has exactly one shape and owns its density.** `StopStreamingButton` takes no `shape`, `fullWidth`, or `className` prop. Call sites vary label and icon scale only.
